...
class RGTGTrainer(Trainer):
    ...
    def __init__(
        ...
        #print('=========')
        # wandb.init(project='rgtg', entity='huggingface', config=args, tags=[])
        super().__init__(
            model=model,
            args=args,
    ...
    def rgtg_forward(
        ...
        greedy_output = model.module.generate(**chat_inputs.to(self.accelerator.device), max_new_tokens=512, num_return_sequences=1, do_sample=False, temperature=1, use_cache=False)
        ...
        with open(self.args.output_dir + '/greedy_output.jsonl','a') as f:
            ...
            f.write('\n')
        greedy_assistant = tokenizer.decode(greedy_output[0][len(chat_inputs[0]):])
        self.store_metrics({'generation_length':float(len(greedy_output[0])-len(chat_inputs[0]))}, train_eval="train")

        # batch = chat_inputs + greedy_assistant
        batch = greedy_output
        ...
        all_logits = model(
            input_ids = batch.to(self.accelerator.device),
            return_dict=True,
        ).logits
        
        # find_char = '<|assistant|>\n'
        # chat_prompt = batch_text[:batch_text.index(find_char)]
        # chat_inputs = tokenizer(chat_prompt, return_tensors='pt').to(model.device)
        start_response = len(chat_inputs.input_ids[0])
        
        prescreen_logits, prescreen_tokens = torch.topk(all_logits, dim=-1, k=self.topk)
        end_response = prescreen_logits.shape[1]
        
        student_logits = all_logits[:,range(end_response-1,start_response-1,-self.window_size)]
        # rgtg_logits = 
        rgtg_logits = student_logits.clone().detach().to(student_logits.device)
        rgtg_logits.fill_(-float('inf'))
        self.ref_model.eval()
        if self.use_ref:
            with torch.no_grad():
            ...
            ref_logits = ref_logits[:,range(end_response-1,start_response-1,-self.window_size)]
        ...
        with torch.no_grad():
            rm_output = rm_model(input_ids = batch.to(self.accelerator.device),return_dict=True, use_cache=True)
        # print(f"rm_output: {rm_output.logits.item()}")
        metrics = {'rewards': rm_output.logits.item()}
        self.store_metrics(metrics, train_eval="train")

        past_key_values_all = self.duplicate_kv_cache(rm_output.past_key_values,self.topk)
        cur_rgtg_weight = self.rgtg_weight
        if self.adapt_weight:
            cur_rgtg_weight = max(0.1,min(cur_rgtg_weight,cur_rgtg_weight * (1.5-rm_output.logits.item())))
        
        cnt_target =0
        for idx in range(end_response-1,start_response-1,-self.window_size):
            with torch.no_grad():
                cnt_target+=1
                cur_response_ids = prescreen_tokens[0:1,idx,:]
                llm_inputs_candidates = torch.concat((batch[:,:idx+1].repeat(self.topk,1).to(rm_model.device),cur_response_ids.T.to(rm_model.device)),dim=1)

                # cur_response=tokenizer.batch_decode(llm_inputs_candidates[:,chat_inputs.input_ids.shape[1]:])
                cur_rm_inputs = llm_inputs_candidates
                # cur_rm_inputs = torch.concat((llm_inputs_candidates,torch.IntTensor([[tokenizer.eos_token_id]]).to(llm_inputs_candidates.device).repeat(self.topk,1)),dim=1)
                del llm_inputs_candidates
                torch.cuda.empty_cache()
                
                cache_pos = cur_rm_inputs.shape[1]-2
                ##get partial_kv_cache
                past_key_values_partial = self.partial_kv_cache(past_key_values_all,cache_pos)
                ...
                cur_guided_score = prescreen_logits[0,idx,:].detach() + cur_rgtg_weight * rm_scores[:,0].to(prescreen_logits.device)
                # rgtg_logits[:,rgtg_logits.shape[1]-cnt_target,prescreen_tokens[0,idx-1,:]] = cur_guided_score.to(rgtg_logits.device)
                rgtg_logits[:,cnt_target-1,prescreen_tokens[0,idx,:]] = cur_guided_score.to(rgtg_logits.device)

                del rm_model_outputs, cur_guided_score,past_key_values_partial
                torch.cuda.empty_cache()
        del (cur_rm_inputs,rm_scores,past_key_values_all)
        torch.cuda.empty_cache()    
