...
from trl.trainer.ppov2_trainer import INVALID_LOGPROB, PPOv2Config
...
from utils import batch_generation, get_score
...
class PPOTrainer(Trainer):
    ...
    def __init__(
        ...
        # Revert what the PPOv2Trainer does to the policy generation config for whatever reason.
        # self.policy.generation_config.eos_token_id = tokenizer.eos_token_id
        # self.policy.generation_config.pad_token_id = tokenizer.pad_token_id
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            with torch.no_grad():
                ...
                for i in range(0, queries.shape[0], args.local_rollout_forward_batch_size):
                    ...
                    postprocessed_response_text = tokenizer.batch_decode(postprocessed_response, skip_special_tokens=False)
                ...
                # TODO(kykim): This check isn't technically correct but easy to implement. What is more
                # correct is to check if the response ends with an eos optionally followed by pads.
        ...
        with unwrap_model_for_generation(self.model, self.accelerator) as unwrapped_model:
            for batch in self.eval_dataloader:
                ...
                with torch.no_grad():
                    ...
                    if args.non_eos_penalty:
                        score = torch.where(contain_eos_token, score, args.penalty_reward_value)
                    if args.empty_output_penalty:
                        sequence_length = first_true_indices(postprocessed_response == tokenizer.pad_token_id) - 1
                        non_empty_responses = torch.gt(sequence_length, 0)
                        score = torch.where(non_empty_responses, score, args.penalty_reward_value)
