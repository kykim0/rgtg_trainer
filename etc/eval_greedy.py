...
# parser.add_argument("--dataset", type=str, default="allenai/ultrafeedback_binarized_cleaned")
...
def generate(query_texts, model, tokenizer, rmodel, rtokenizer, generation_config):
    ...
    with torch.no_grad():
        ...
        sequence_lengths = first_true_indices(outputs[:, context_length:] == tokenizer.pad_token_id) - 1
    ...
    return output_dict, output_texts, sequence_lengths, scores
...
def main():
    ...
    for idx in tqdm(range(0, len(dataset), batch_size)):
        ...
        if ref_model:
            ...
            ref_logits = ref_output.logits[:, context_length:]
            klds = kld(logits, ref_logits)
